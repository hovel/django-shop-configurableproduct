...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for t in ProductType.objects.all():
            for bf in t.boolean_fields.all():
                TypeBoolean.objects.create(field=bf, type=t)
            for cf in t.char_fields.all():
                TypeChar.objects.create(field=cf, type=t)
            for ff in t.float_fields.all():
                TypeFloat.objects.create(field=ff, type=t)
            for fi in t.image_fields.all():
                TypeImage.objects.create(field=fi, type=t)


    def backwards(self, orm):
        raise Exception('Data migration can\'t be reversed')
